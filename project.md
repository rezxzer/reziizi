...
- **Supabase SQL (migrations):** production-ზე **დადასტურებულია** თანმიმდევრობა **`20260401120000` … `20260401352500_add_ad_slots_video_url_and_storage.sql`** (სრული ცხრილი — **`reziizi.mdc`**), მათ შორის **2026-04-11** ჩატში დადასტურებული: **`20260401352300_free_tier_one_video_per_utc_day.sql`**, **`20260401352400_add_ad_placement_requests.sql`**, **`20260401352500_add_ad_slots_video_url_and_storage.sql`**.
- **სპონსორის განაცხადები (feed top):** migration `20260401352400_add_ad_placement_requests.sql` — `ad_placement_requests` + RLS; აპი: **`/sponsored`** (განაცხადი + საკუთარი სტატუსი), **`/admin/ad-requests`** (რიგი); მთავარი ლენტის **`Sponsored`** ლეიბლი **ინტერნალურად** მიდის `/sponsored`-ზე; ცოცხალი ტექსტი რეკლამის სლოტში კვლავ **`/admin/ads`**. **UI:** ამ სამ გვერდზე ჩანს **`platform-test-notice`** — **სატესტო რეჟიმი** (ფასიანი განთავსება/ფული ჯერ არა; დალოდება პროდუქტის განახლებამდე). **Production:** ✅ migration **გაშვებულია** Supabase-ზე (დადასტურებული).
- **Feed-top სარეკლამო ვიდეო (ადმინი):** migration `20260401352500_add_ad_slots_video_url_and_storage.sql` — `ad_slots.video_url`, bucket **`feed-ad-videos`** (მხოლოდ ადმინის upload `feed_top/*`); **`/admin/ads`** — MP4/WebM ატვირთვა; **`FeedAdSlot`** — `<video controls>`. **Production:** ✅ **გაშვებულია** Supabase-ზე (Storage bucket + პოლიტიკები; დადასტურებული).
...
- **Social privacy/moderation გაუმჯობესებები (Claude, 2026-04-09):** private პროფილი + follow requests + blocks + user reports + last seen. **DB:** ახალი migrations `20260401351800`…`20260401352100` (private profiles / `follow_requests`, `blocks`, `user_reports`, `profiles.last_seen_at` + `update_last_seen`) **წარმატებით გაშვებულია production Supabase-ზე** (დადასტურებული). **App:** `src/lib/followRequests.ts`, `src/lib/blocks.ts`, `src/lib/userReports.ts`, `src/lib/lastSeen.ts` + შესაბამისი UI ინტეგრაციები `ProfilePage` / `UserProfilePage` / `SettingsPage` / `MessagesPage` / `NotificationsPage` / `SearchPage` / `Layout`.
- **Discoverability + moderation expansion (Claude, 2026-04-09):** **Trending tags RPC** (`20260401352200_add_trending_tags_rpc.sql`, ფუნქცია `public.trending_tags`) + `src/lib/trendingTags.ts` და `HomePage` sidebar ინტეგრაცია; **Admin user-level moderation queues** — `/admin/user-reports`, `/admin/blocks` (`AdminUserReportsPage`, `AdminBlocksPage`, `adminStats`/`AdminPage` sync); **Post edit flow** — `src/lib/editPost.ts` + `PostCard` inline edit/save/cancel + edited indicator. **სტატუსი:** კოდი რეპოში და SQL migration live Supabase-ზე **დადასტურებულია**.
...
- **ადმინ UI polish (2026-04):** Dashboard (nav კარტები, metrics grid), Users (ძებნა, ჩიპები, pagination, mobile cards), Moderation (მედია preview, flagged ფილტრი), Reports (report count, hot highlight), Stats (K/M ფორმატი), Feature flags (CSS) — დეტალი **`JOURNAL.md` → 2026-04-07 (Admin pages)**; **`AdminAdsPage` / `AdminApiPage`** იმავე ტალღაში **არ შეცვლილა**.
- **მთავარი (`/`) — polish სკოპი Cowork-ისთვის:** **`HOME_PAGE_POLISH.md`** (ინგლისური) — რა არის იმპლემენტირებული, რა არ დაირღვეს, უსაფრთხო მიმართულებები; პატარა ცვლილებები — **`JOURNAL.md` → 2026-04-07 (Home)**.
...
- **ინფინიტი სქროლი (UI, 2026-04-08):** **`src/hooks/useInfiniteScroll.ts`** — `IntersectionObserver` სენტინელი; გამოყენება: **`HomePage`**, **`ProfilePage`** (Commented ტაბი), **`UserProfilePage`** (Commented), **`UserFollowListPage`** — ხელით „Load More“ ღილაკი ამ სიებში აღარა; იხილე **`JOURNAL.md` → 2026-04-08**.
- **ტალღა 2 — Motion / დიზაინი: პრიორიტეტი და დადასტურება (2026-04-09):** გარე ასისტენტის (Claude) შემოთავაზებული **motion + sound + route transitions** ხაზი არის **ოფიციალური პრიორიტეტი** შემდგომი UI polish-ისთვის. პროდუქტმა **დაადასტურა** ვიზუალური მიმართულება და გვერდებს შორის გადასვლა. **ტექნიკა:** `LayoutOutlet.tsx`, `styles.css`; **სპეკი:** **`MOTION_SOUND_POLISH.md`** (Tier 1–3, route/sound map, `prefers-reduced-motion`). **ქრონოლოგია / ერთიანი ჩანაწერი (ძველი დაშლილი ჩანაწერები იქ გაერთიანდა):** **`JOURNAL.md` → 2026-04-09**.
- **ფაილის ინფუტი + CSS polish (ასისტენტის სესია, 2026-04):** ფაილის ინფუტის overlap-ის თავიდან ასაცილებლად (Post/Avatar) და კომპონენტური კლასების დაფარვა redesign-ის შემდეგ — **`JOURNAL.md` → 2026-04-07**; ლოკალური `styles.css` / `package.json` ცვლილებების **commit** საჭიროა სრული სინქისთვის remote-თან.
...
| 16 | **Motion + Sound system (UI polish)** — Claude-ის შეთანხმებული ხაზი | ✅ **პრიორიტეტად დადასტურებული** პროდუქტის მხრიდან; Tier 1–3 იმპლემენტაცია კოდში — 🔄 დახვეწა/QA სურვილისამებრ | **`MOTION_SOUND_POLISH.md`**; სტატუსის ერთი წყარო — **`JOURNAL.md` → 2026-04-09** |
| 17 | **Privacy + moderation expansion** (private profiles, follow requests, blocks, user reports, last seen) | ✅ **კოდი რეპოში + migrations live Supabase-ზე გაშვებულია** (დადასტურებული) | migrations `20260401351800`…`20260401352100`; `JOURNAL.md` → 2026-04-09 |
| 18 | **Trending tags + admin user moderation queues + post edit** | ✅ **კოდი რეპოში + migration live Supabase-ზე გაშვებულია** (დადასტურებული) | `20260401352200_add_trending_tags_rpc.sql`; `/admin/user-reports`; `/admin/blocks`; `src/lib/editPost.ts`; `JOURNAL.md` → 2026-04-09 |
