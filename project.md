...
- **Supabase SQL (migrations):** production-ზე **დადასტურებულია** თანმიმდევრობა **`20260401120000` … `20260401352200_add_trending_tags_rpc.sql`** (იხილე **`reziizi.mdc`** სრული ცხრილი). **რეპო + GitHub (`main`):** დამატებულია **`20260401352300_free_tier_one_video_per_utc_day.sql`**, **`20260401352400_add_ad_placement_requests.sql`**, **`20260401352500_add_ad_slots_video_url_and_storage.sql`** — სამივე **გაუშვი production Supabase-ზე** (SQL Editor ან `supabase db push` link-ზე), სანამ შესაბამისი აპის ფიჩები live-ზე გჭირდებათ; დადასტურების შემდეგ განაახლე ეს ხაზი + **`JOURNAL.md`**.
