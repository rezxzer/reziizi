-- REZIIZI: Anti-spam (49) — duplicate heuristic only for non-trivial bodies.
-- Short repeated text (e.g. "ok", "+1") no longer triggers duplicate flag within the window.
-- Phase A refinement; see project.md → Anti-spam (49).
...
immutable
as $$
  select length(public.normalize_body_for_spam(p)) >= 12;
$$;

comment on function public.spam_duplicate_eligible(text) is
  'Duplicate-within-window anti-spam applies only when normalized body length >= 12 (reduces false positives on very short posts).';
